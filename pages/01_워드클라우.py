...
# 워드클라우드에서 한글이 깨지지 않게 사용할 나눔고딕 폰트 주소
...
# ------------------------------------------------------------
# 댓글 전체를 단어로 쪼개서 빈도수를 세는 함수 (공용)
# ------------------------------------------------------------
def build_word_counter(comments: list):
    """
    댓글 목록의 "댓글" 텍스트를 모두 단어로 쪼갠 뒤,
    한 글자짜리 단어는 빼고 전체 등장 횟수를 세어 Counter로 돌려줍니다.
    이 결과는 상위 20개 막대그래프와 워드클라우드에서 함께 사용합니다.
    """
    word_counter = Counter()

    for comment in comments:
        text = comment.get("댓글", "")
        # 한글, 영문, 숫자로 이루어진 덩어리를 "단어"로 봅니다.
        # (이모지, 특수문자, 자음/모음만 있는 자모 등은 자동으로 걸러집니다.)
...
def count_top_words(comments: list, top_n: int = 20):
    """
    build_word_counter로 구한 단어 빈도 중 상위 top_n개를 돌려줍니다.
    반환값: [(단어, 등장횟수), ...] 형태의 리스트 (자주 나온 순)
    """
    return build_word_counter(comments).most_common(top_n)


# ------------------------------------------------------------
# 워드클라우드용 한글 폰트를 내려받는 함수
# ------------------------------------------------------------
@st.cache_resource
def download_font():
    """
    나눔고딕 폰트 파일을 내려받아 임시 폴더에 저장하고 경로를 돌려줍니다.
    내려받기에 실패하면 None을 돌려줍니다.
    (st.cache_resource로 캐싱해서 한 번만 내려받습니다.)
    """
    try:
        ...
        if not api_key:
            ...
            if error_message:
                ...
                if not top_words:
                    ...
                    # px.bar는 기본적으로 첫 항목을 그래프 맨 아래에 그리기 때문에,
                    # y축을 뒤집어서 가장 많이 나온 단어가 맨 위로 오게 합니다.
                ...
                if not font_path:
                    ...
                    word_freq = build_word_counter(comments_sorted)

                    if not word_freq:
                        st.info("워드클라우드로 그릴 만한 단어를 찾지 못했어요.")
                    else:
                        wordcloud = WordCloud(
                            font_path=font_path,
                        ...
                        # matplotlib 없이 wordcloud가 만든 이미지를 그대로 화면에 띄웁니다.
